...
def learn(scene_id, num_steps):
    ...
    model_file=tf.train.latest_checkpoint('{}/../ckpt/'.format(root_folder))
    if model_file != None:
        print("restored saved file")
    ...
    for timestep in range(num_steps):
        ...
        if ((sum(seg['wins']) / len(seg['wins'])) + 1) / 2 > EloHelper.updateThreshold:
            ...
            if g_save_pb_model:
                ...
                for root, _, files in os.walk('{}/ckpt/model_{}'.format(root_folder, idx)):
                    for file_name in files:
                        full_file_name = '{}/{}'.format(root, file_name)
                        response = s3.meta.client.upload_file(full_file_name, mybucket, 'ckpt/model_{}/{}'.format(idx, file_name))
                        print(response)
                for root, _, files in os.walk('{}/model/model_{}'.format(root_folder, idx)):
                    for file_name in files:
                        full_file_name = '{}/{}'.format(root, file_name)
                        response = s3.meta.client.upload_file(full_file_name, mybucket, 'model/model_{}/{}'.format(idx, file_name))
                        print(response)
        ...
        saver.save(session, '{}/../ckpt/mnist.ckpt'.format(root_folder), global_step = 0)#g_step + 1)
